...
def handle_queue_status(*, db_path: Path, days: int = 7, limit: int = 10) -> str:
    ...
    try:
        rows = conn.execute(
            "SELECT title, source, entities_json, collected_at FROM articles WHERE collected_at >= ? ORDER BY collected_at DESC LIMIT ?",
            [cutoff, limit * 5],
        ).fetchall()
    finally:
        conn.close()

    queue_keywords = {"wait time", "대기", "queue", "reservation", "예약", "no-show", "노쇼"}
    alerts: list[str] = []
    for title, source, entities_json, collected_at in rows:
        title_lower = str(title).lower()
        if any(keyword in title_lower for keyword in queue_keywords):
            entity_info = ""
            if entities_json:
                try:
                    entities = json.loads(str(entities_json))
                    if isinstance(entities, dict) and entities:
                        entity_info = f" [{', '.join(str(key) for key in entities.keys())}]"
                except json.JSONDecodeError:
                    pass
            alerts.append(f"- {title}{entity_info} ({source}, {collected_at})")

    if not alerts:
        return "No queue or reservation alerts found."
    return f"Queue status ({len(alerts)} item(s) in last {days} days):\n" + "\n".join(
        alerts[:limit]
    )
